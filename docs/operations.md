...
`delta-algo-graph-flow` is centered on algorithms work in OCaml. The technical target is: Package an OCaml local lab for graph analysis with framed sample traffic, bounds and ordering tests, and documented operating limits.
...
The score is deterministic. The current threshold is 180. Risk has a penalty of 6, latency has a penalty of 2, and weight contributes a bonus of 3.
